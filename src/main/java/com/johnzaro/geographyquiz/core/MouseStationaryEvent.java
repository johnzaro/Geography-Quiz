...
public abstract class MouseStationaryEvent extends InputEvent
{
	...
	private static class Begin extends MouseStationaryEvent
	{
		...
		{
			if(source instanceof Node) return ((Node) source).screenToLocal(screenPos);
			else if(source instanceof Scene) return getScenePosition();
			else return null;
		}
		...
		public Point2D getScenePosition()
		{
			...
			if(source instanceof Node) scene = ((Node) source).getScene();
			else if(source instanceof Scene) scene = (Scene) source;
			else return null;
